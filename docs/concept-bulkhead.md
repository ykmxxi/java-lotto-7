...
- 로또: 1급 개념
  - 로또 번호: 상태
- 로또 발매: 2급 개념, 로또가 알고 있으면 안되는 개념, 발행한 숫자들만 넘겨준다(외부로 분리)
  - 자동 발매: 6개의 번호를 자동으로 발매, 로또 번호의 완벽한 제어/통제 불가능
  - 수동 발매: 6개의 번호를 수동으로 발매, 완벽한 제어/통제 가능
- 로또들: 사용자가 구매한 1개 이상의 로또, 2급 개념
- 당첨: 1급 개념, 당첨은 **돈 계산 관련 도메인**이다. 즉, 성능/효율보다 **안전성**을 최우선시 해야 한다
  - 등수와 당첨 금액: 상태, 고유한 값을 유지
  - 당첨 번호들(6개의 당첨 번호와 1개의 보너스 번호): 상태
  - 당첨 여부 확인: 행위
  - 등수, 총 당첨 금액 계산: 행위
  - 수익률 계산: 행위
    - 수익률을 굳이 상태로 둘 필요는 없음. 단, 0%는 자주 발생할 확률이 높아 캐싱해서 돌려줄만 함
...
- 간단한 로또 발매기를 구현한다.
  ...
  - 발매기: 핵심 개념? 핵심 개념이라기 보단 핵심 개념 로또를 발행할 때 사용하는 방법(전략?)
...
- 로또 1장의 가격은 1,000원이다.
  - 몇 장 까지 구매가 가능할까? 일단 프로그램 실행 시 1회성 구매 -> 1개 발행 매장에서 구매한 것과 비슷 
    ...
    - 토스, 삼쩜삼 같은 서비스 기업에서 이벤트로 진행하는 로또: 기간(하루, 1주) 발행량은 제한이 없으나, 현금이 들지 않나 얻기 힘듬
  - 100장 제한
    ...
    - 시스템 한계 -> 만약 21억 까지 구매가 가능하다면? 2100만장 로또는 약 1억 2600만 개의 번호가 생성됨, 출력시 문제
      - Integer 6개는 약 120bytes, List 구조체는 약 68bytes, 따라서 로또 1장 당 약 188bytes -> 21억개는 3.67GB 정도의 메모리를 잡아먹음
      - 100장 제한하면 약 19KB
